...
export async function updateProfile(userId, { color, avatarUrl, bannerUrl, bannerStyle, avatarDecoration, profileEffect, profileTheme, nameStyle, profileFrame, nameplateStyle, profilePrimaryColor, profileSecondaryColor, showLastSeen, bio, birthDate, displayName, username }) {
  ...
  const bannerStyles = new Set(['profile', 'ocean', 'sunset', 'aurora', 'midnight', 'berry', 'gold', 'mono'])
  const avatarDecorations = new Set(['none', 'ring', 'neon', 'sparkles', 'double', 'halo', 'petals', 'flames', 'bubbles', 'pixel', 'frost', 'vines'])
  const profileEffects = new Set(['none', 'glow', 'aurora', 'grid', 'holo', 'stars', 'confetti', 'rain', 'pulse', 'scan', 'sakura', 'comet'])
  const profileThemes = new Set(['default', 'night', 'berry', 'ocean', 'forest'])
  const nameStyles = new Set(['plain', 'accent', 'gradient', 'glow', 'mono'])
  const profileFrames = new Set(['none', 'accent', 'glass', 'gold', 'neon'])
  const nameplateStyles = new Set(['none', 'cosmic', 'sakura', 'arcade', 'forest', 'gold'])
